...
class fts_config(TransientModel):
    ...
    def _get_default_form_view(self, cr, user, context=None):
        ...
        for search_plugin in fts_base_meta._plugins:
            group = etree.SubElement(
                view, 'group', string=search_plugin._model, colspan='2',
                col='3')
            etree.SubElement(
                group, 'field', name='tsconfig_'+search_plugin._model,
                invisible='True')
            etree.SubElement(
                group, 'field', name='tsconfig_'+search_plugin._model)
            etree.SubElement(
                group, 'button', type="object", name="recreate_search_index",
    ...
    def recreate_search_index(self, cr, uid, ids, context=None):
        for search_plugin in fts_base_meta._plugins:
            if search_plugin._model == context.get(
                    'recreate_search_index_model'):
                self.pool.get('fts.proxy').recreate_search_index(
                    cr, uid, search_plugin)
